/*
  # استبدال عمود is_admin بعمود role بشكل كامل

  1. المشاكل المحلولة
    - حذف جميع السياسات التي تعتمد على عمود is_admin
    - استبدال عمود is_admin بعمود role
    - إعادة إنشاء السياسات باستخدام نظام الأدوار الجديد

  2. التغييرات
    - حذف جميع السياسات المعتمدة على is_admin
    - إضافة عمود role إذا لم يكن موجوداً
    - تحويل البيانات من is_admin إلى role
    - حذف عمود is_admin
    - إعادة إنشاء السياسات باستخدام role
*/

-- الخطوة 1: حذف جميع السياسات التي تعتمد على عمود is_admin
...
-- الخطوة 4: حذف عمود is_admin الآن بعد حذف السياسات
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
...
-- الخطوة 5: إنشاء فهرس على عمود role
CREATE INDEX IF NOT EXISTS idx_user_profiles_role ON user_profiles(role);

-- الخطوة 6: إعادة إنشاء السياسات باستخدام نظام الأدوار الجديد
...
-- الخطوة 7: تحديث دالة إنشاء المستخدم الجديد
...
-- الخطوة 8: تحديث دالة تحديث دور المستخدم
...
-- الخطوة 9: حذف وإعادة إنشاء الـ View
...
-- الخطوة 10: إنشاء دوال مساعدة للتحقق من الأدوار
...
-- الخطوة 11: تحديث دالة حذف كود الاشتراك
